...
app.config['NEO4J_USER'] = "neo4j"  # Replace with your Neo4j user
app.config['NEO4J_PASSWORD'] = "password"  # Replace with your Neo4j password
...
def search():
    ...
    try:
        ...
        with driver.session() as session:
            ...
            """
            results = session.run(query, pattern=f'.*{search_term}.*') # Neo4j LIKE equivalent using =~
            nodes = [record['node'] for record in results]

            # Filter results further in Python using re2 for more complex regex if needed
            filtered_nodes = [node for node in nodes if pattern.search(node['name']) or pattern.search(node['description'])]


            return jsonify(nodes=filtered_nodes) # Returning unfiltered nodes for now as Neo4j's =~ does basic pattern match
    ...
    try:
        with driver.session() as session:
            query = """
                MATCH (n)-[r]->(m)
                RETURN
                    { id: id(n), name: n.name, label: labels(n) } AS source,
                    { id: id(m), name: m.name, label: labels(m) } AS target,
